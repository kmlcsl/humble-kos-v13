...
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('pemilik.kamar.index') }}">Manajemen Kamar</a></li>
...
@section('page-actions')
    <div class="btn-group" role="group">
        <a href="{{ route('pemilik.kamar.index') }}" class="btn btn-secondary">
...
@section('content')
    ...
    <form action="{{ route('pemilik.kamar.update', $kamars->kamar_id) }}" method="POST" enctype="multipart/form-data"
        novalidate>
        @csrf
        @method('PUT')
        @if (session('success'))
        ...
        <div class="row">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('tipe_kamar') is-invalid @enderror" id="tipe_kamar"
                                    ...
                                    <option value="single"
                                        {{ old('tipe_kamar', $kamars->tipe_kamar) == 'single' ? 'selected' : '' }}>Single
                                    </option>
                                    <option value="double"
                                        {{ old('tipe_kamar', $kamars->tipe_kamar) == 'double' ? 'selected' : '' }}>Double
                                    </option>
                                    <option value="shared"
                                        {{ old('tipe_kamar', $kamars->tipe_kamar) == 'shared' ? 'selected' : '' }}>Shared
                                    </option>
                                </select>
                                @error('tipe_kamar')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('kapasitas') is-invalid @enderror"
                                    id="kapasitas" name="kapasitas" value="{{ old('kapasitas', $kamars->kapasitas) }}"
                                    min="1" required>
                                @error('kapasitas')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('ukuran_kamar') is-invalid @enderror"
                                id="ukuran_kamar" name="ukuran_kamar"
                                value="{{ old('ukuran_kamar', $kamars->ukuran_kamar) }}"
                                placeholder="Contoh: 3x4 m atau 12 m²" required>
                            @error('ukuran_kamar')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number"
                                        ...
                                        id="harga_per_bulan" name="harga_per_bulan"
                                        value="{{ old('harga_per_bulan', $kamars->harga_per_bulan) }}" min="0"
                                        required>
                                </div>
                        ...
                        <div class="row">
                            @php
                                $selectedFasilitas = old(
                                    'fasilitas',
                                    $kamars->fasilitas->pluck('fasilitas_id')->toArray(),
                                );
                            @endphp
                                ...
                                <div class="col-md-4 col-6 mb-3">
                                    <div class="form-check">
                                        ...
                                        <label class="form-check-label" for="fasilitas_{{ $item->fasilitas_id }}">
                                            <i
                                                class="{{ $item->icon_fasilitas ?? 'fas fa-check' }} me-1 text-primary"></i>
                                            {{ $item->nama_fasilitas }}
                                        </label>
                                    </div>
                                </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-4">
                            ...
                            @if ($kamars->foto_kamar)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $kamars->foto_kamar) }}" alt="Foto Utama"
                            ...
                            @endif
                            <div class="mt-2">
                                <label for="foto_kamar" class="form-label">{{ $kamars->foto_kamar ? 'Ganti' : 'Upload' }}
                                    Foto Utama</label>
                                <input type="file" class="form-control @error('foto_kamar') is-invalid @enderror"
                                    id="foto_kamar" name="foto_kamar" accept="image/*">
                                @error('foto_kamar')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div>
                            ...
                            @if ($kamars->fotos && $kamars->fotos->isNotEmpty())
                                        ...
                                        <div class="col-md-6 col-lg-3">
                                            <div class="position-relative">
                                                <img src="{{ asset('storage/' . $foto->path_foto) }}"
                                                    alt="Foto {{ $foto->urutan }}" class="img-thumbnail"
                                                    style="width: 100%; height: 150px; object-fit: cover;">
                                                <div
                                                    class="form-check position-absolute bottom-0 inset-e-0 m-2 bg-white rounded px-2 py-1">
                                                    <input class="form-check-input" type="checkbox" name="hapus_foto[]"
                                                        value="{{ $foto->foto_id }}"
                                                        id="hapus_foto_{{ $foto->foto_id }}">
                                                    <label class="form-check-label small text-danger"
                                                        for="hapus_foto_{{ $foto->foto_id }}">
                                                        Hapus
                                                    </label>
                                                </div>
                                            </div>
                                ...
                                <div class="form-text mt-2">Centang "Hapus" untuk menghapus foto yang tidak diinginkan saat
                                    update.</div>
                            @else
            ...
            <div class="col-lg-4">
                ...
                <div class="admin-pemilik-card mb-4">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            <label for="status_kamar" class="form-label">Status <span
                                    class="text-danger">*</span></label>
                            <select class="form-select @error('status_kamar') is-invalid @enderror" id="status_kamar"
                                name="status_kamar" required>
                                <option value="tersedia"
                                    {{ old('status_kamars', $kamars->status_kamar) == 'tersedia' ? 'selected' : '' }}>
                                    Tersedia
                                </option>
                                <option value="terisi"
                                    {{ old('status_kamar', $kamars->status_kamar) == 'terisi' ? 'selected' : '' }}>Terisi
                                </option>
                                <option value="maintenance"
                                    {{ old('status_kamar', $kamars->status_kamar) == 'maintenance' ? 'selected' : '' }}>
                                    Maintenance</option>
                            </select>
                            @error('status_kamar')
                                <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="admin-pemilik-card mb-4">
                    <div class="card-body">
                        ...
                        <a href="{{ route('pemilik.kamar.index') }}" class="btn btn-outline-secondary w-100 mt-2">
