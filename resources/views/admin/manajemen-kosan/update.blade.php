...
@section('page-actions')
    <div class="btn-group" role="group">
        <a href="{{ route('admin.manajemen-kosan.show', $kosan->kosan_id) }}" class="btn btn-info">
...
<form action="{{ route('admin.manajemen-kosan.update', $kosan->kosan_id) }}" method="POST" enctype="multipart/form-data" id="editKosanForm">
    @csrf
    @method('PUT')
            ...
            <div class="admin-pemilik-card mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('nama_kosan') is-invalid @enderror" id="nama_kosan" name="nama_kosan" value="{{ old('nama_kosan', $kosan->nama_kosan) }}" required>
                        @error('nama_kosan')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi" rows="5" required>{{ old('deskripsi', $kosan->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <div class="form-text">Deskripsikan kosan secara detail, termasuk keunggulan dan informasi penting lainnya.</div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('jenis_kos') is-invalid @enderror" id="jenis_kos" name="jenis_kos" required>
                                <option value="">Pilih Jenis Kosan</option>
                                <option value="putra" {{ old('jenis_kos', $kosan->tipe_kosan) == 'putra' ? 'selected' : '' }}>Kos Putra</option>
                                <option value="putri" {{ old('jenis_kos', $kosan->tipe_kosan) == 'putri' ? 'selected' : '' }}>Kos Putri</option>
                                <option value="campur" {{ old('jenis_kos', $kosan->tipe_kosan) == 'campur' ? 'selected' : '' }}>Kos Campur</option>
                            </select>
                            @error('jenis_kos')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('id_pemilik') is-invalid @enderror" id="id_pemilik" name="id_pemilik">
                                <option value="">Pilih Pemilik (Opsional)</option>
                                @foreach($owners ?? [] as $owner)
                                    <option value="{{ optional($owner)->user_id }}" {{ old('id_pemilik', $kosan->owner_id) == optional($owner)->user_id ? 'selected' : '' }}>{{ optional($owner)->name }} ({{ optional($owner)->username }})</option>
                                @endforeach
                            </select>
                            @error('id_pemilik')
                                <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="admin-pemilik-card mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('alamat') is-invalid @enderror" id="alamat" name="alamat" rows="2" required>{{ old('alamat', $kosan->alamat) }}</textarea>
                        @error('alamat')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('kota') is-invalid @enderror" id="kota" name="kota" value="{{ old('kota', $kosan->kota) }}" required>
                        @error('kota')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('latitude') is-invalid @enderror" id="latitude" name="latitude" value="{{ old('latitude', $kosan->latitude) }}">
                            @error('latitude')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('longitude') is-invalid @enderror" id="longitude" name="longitude" value="{{ old('longitude', $kosan->longitude) }}">
                            @error('longitude')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">Klik pada peta untuk menentukan lokasi kosan. Koordinat akan terisi otomatis.</div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="admin-pemilik-card mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status_validasi') is-invalid @enderror" id="status_validasi" name="status_validasi">
                            <option value="pending" {{ old('status_validasi', $kosan->status_validasi) == 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="approved" {{ old('status_validasi', $kosan->status_validasi) == 'approved' ? 'selected' : '' }}>Disetujui</option>
                            <option value="rejected" {{ old('status_validasi', $kosan->status_validasi) == 'rejected' ? 'selected' : '' }}>Ditolak</option>
                        </select>
            ...
            <div class="admin-pemilik-card mb-4">
                ...
                <div class="card-body">
                    ...
                    <p class="mb-2"><strong>Rating:</strong> {{ number_format($kosan->rating_rata ?? 0, 1) }}/5.0</p>
                    <p class="mb-2"><strong>Jumlah Kamar:</strong> {{ $kosan->kamars->count() }} kamar</p>
                    <p class="mb-0"><strong>Dibuat:</strong> {{ $kosan->created_at ? $kosan->created_at->format('d M Y') : '-' }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    function previewAdditionalImages(input) {
        ...
        if (input.files) {
            ...
            if (input.files.length > maxFiles) {
                alert(`Maksimal ${maxFiles} foto tambahan. Foto yang dipilih: ${input.files.length}. Hanya ${maxFiles} foto pertama yang akan digunakan.`);
            }
        }
    }
            ...
            if ((force || (!latField.value && !lngField.value)) && navigator.geolocation) {
                ...
                navigator.geolocation.getCurrentPosition(function(position) {
                    ...
                    if (error.code === error.PERMISSION_DENIED) {
                        errorMessage = 'Anda menolak izin lokasi. Mohon izinkan akses lokasi di pengaturan browser.';
                ...
                if (force) {
                 ...
                 alert("Koordinat Latitude dan Longitude sudah terisi. Anda bisa menghapus nilai untuk mendapatkan lokasi baru.");
            }
        }
