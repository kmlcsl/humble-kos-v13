...
<form action="{{ route('admin.fasilitas.update', $fasilitas->fasilitas_id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="row">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('deskripsi') is-invalid @enderror"
                            ...
                            placeholder="Deskripsi fasilitas (opsional)">{{ old('deskripsi', $fasilitas->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $fasilitas->icon_fasilitas) }}"
                                alt="{{ $fasilitas->nama_fasilitas }}" class="img-thumbnail" style="max-height: 150px;">
                            <p class="text-muted small mt-1">Icon saat ini</p>
                        </div>
                        @endif
                        ...
                        @error('icon_fasilitas')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <div class="form-text">Format: JPG, PNG, SVG. Ukuran maksimal: 1MB. Kosongkan jika tidak ingin mengubah icon.</div>
                    </div>
        ...
        <div class="col-lg-4">
            <div class="admin-pemilik-card mb-4">
                ...
                <div class="card-body">
                    <p class="text-muted">Perubahan akan langsung tersimpan dan terlihat pada kamar kosan yang menggunakan fasilitas ini.</p>
                </div>
        ...
        if (fileInput) {
            fileInput.addEventListener('change', function() {
                ...
                if (this.files.length > 0) {
                    ...
                    label.textContent = 'Preview icon baru:';
